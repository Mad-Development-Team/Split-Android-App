...
import com.madteam.split.ui.theme.ChooseAvatarDialog
        ...
        DSModalBottomSheet(
            optionsList = mutableListOf(
                ...
                ModalOption(
                    ...
                    action = {
                        viewModel.onEvent(MyUserUIEvent.OnShowProfileImageModalStateChanged(false))
                        viewModel.onEvent(MyUserUIEvent.OnShowChooseAvatarDialogStateChanged(true))
                    }
                )
            ),
            onClose = {
    ...
    Scaffold(
        ...
        Box(
            ...
            MyUserContent(
                ...
                onShowAvatarsImagesDialogChanged = { state ->
                    viewModel.onEvent(MyUserUIEvent.OnShowChooseAvatarDialogStateChanged(state))
                },
...
fun MyUserContent(
    ...
    if (state.showChooseAvatarDialog) {
        Column(
            modifier = Modifier
                .fillMaxSize()
                .background(SplitTheme.colors.neutral.backgroundHeavy.copy(alpha = 0.3f)),
            ...
            ChooseAvatarDialog(
                imagesList = state.avatarImagesUrlsList,
                onChooseAvatar = {
                    onAvatarImageSelected(it)
                },
                onDismiss = {
                    onShowAvatarsImagesDialogChanged(false)
                },
            )
        }
    }
